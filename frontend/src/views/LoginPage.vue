    ...
    <div class="login-container">
      ...
      <div class="form-section">
        <div class="form-container">
          <!-- 友好提示信息 -->
          <div v-if="redirectMessage" class="redirect-notice">
            <el-alert
...
const redirectMessage = computed(() => {
  ...
  const pageName = pageNames[redirectPath.value] || '该页面'
  return `请先登录后访问${pageName}`
})

// 更新重定向路径
const updateRedirectPath = () => {
  redirectPath.value = localStorage.getItem('redirect_after_login') || ''
}

// 监听localStorage变化
const handleStorageChange = (e) => {
  if (e.key === 'redirect_after_login') {
    redirectPath.value = e.newValue || ''
  }
}
...
onMounted(() => {
  updateRedirectPath()
  // 监听storage事件，当localStorage在其他标签页或窗口中被修改时触发
  window.addEventListener('storage', handleStorageChange)
  // 监听页面焦点事件，当用户切换回登录页面时更新重定向路径
  window.addEventListener('focus', updateRedirectPath)
  
  // 定期检查localStorage变化（用于同一标签页内的导航）
  const checkInterval = setInterval(updateRedirectPath, 500)
  
  // 保存清理函数
  window._loginPageCleanup = () => {
    window.removeEventListener('storage', handleStorageChange)
    window.removeEventListener('focus', updateRedirectPath)
    clearInterval(checkInterval)
  }
})

// 组件激活时也更新重定向路径（适用于keep-alive的情况）
onActivated(() => {
  updateRedirectPath()
})
...
// 组件卸载时清理事件监听器
onUnmounted(() => {
  if (window._loginPageCleanup) {
    window._loginPageCleanup()
    delete window._loginPageCleanup
  }
})
