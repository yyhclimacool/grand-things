    ...
    <div class="form-container">
      <div class="form-content">
        ...
        <div class="main-form card fade-in-up">
          ...
          <!-- 微信公众号链接提取 -->
          <div class="wechat-extract-section" v-if="!isEditMode">
            <h3 class="extract-title">
            ...
            <div class="extract-form">
              <el-input
                v-model="wechatUrl"
                placeholder="粘贴微信公众号文章链接，自动提取标题和内容"
                clearable
                :loading="extracting"
              >
                <template #append>
          ...
          >
            <el-form-item label="事件标题" prop="title">
              <el-input
                ...
                placeholder="请输入事件标题，例如：上海将发放智能网联汽车示范运营牌照"
                maxlength="200"
                show-word-limit
            ...
            <el-form-item label="手动标签">
              <el-input
                ...
                placeholder="输入标签，支持中英文逗号、分号分隔（可选）"
                @input="onTagsChange"
              />
              <div class="form-tip">
                <el-icon><InfoFilled /></el-icon>
                系统会自动提取标签，您也可以手动添加额外标签（支持，,；;分隔）
              </div>
            </el-form-item>
...
const allTags = computed(() => {
  const userTags = splitTags(eventForm.value.tags)
...
async function extractWechatContent() {
  if (!wechatUrl.value) {
    ElMessage.warning('请输入微信公众号文章链接')
    return
  }
  ...
  try {
    ...
    // 填充表单数据
    if (data.title) {
      eventForm.value.title = data.title
    }
    
    if (data.content) {
      eventForm.value.description = data.content
    }
